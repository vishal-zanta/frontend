...
export default function AgentStatusBoard({ isSupervisor = false }) {
  const pageProps = usePagination();
  const { data, isLoading, error } = useGetShifts({page : pageProps.page,limit :  pageProps.limit});
    ...
    <div className="bg-white rounded-xl border border-border overflow-hidden">
      <div className="px-5 py-3 border-b border-border">
        <h3 className="font-bold text-foreground">
          {isSupervisor ? "Agent Status Board" : "Agent Status Board (Read-Only)"}
        </h3>
      </div>
          ...
          <tbody className="divide-y divide-border">
            {shiftsData.map((a) => (
              <tr key={a._id} className="hover:bg-muted/30">
                ...
                <td className="px-4 py-3 text-muted-foreground">{a.role?.level || a.role?.designationEnglish}</td>
                <td className="px-4 py-3 text-muted-foreground">{formatShift(a?.shift)}</td>

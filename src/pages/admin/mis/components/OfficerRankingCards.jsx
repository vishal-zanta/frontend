...
export default function OfficerRankingCards({ reportRows = [] }) {
  const { t } = useLanguage();
      ...
      {reportRows.map((row, i) => (
        ...
        >
          <div className="p-3 xs:p-3.5 border-b border-border/60 flex items-center justify-between gap-2">
            ...
            <Badge variant="outline" className="text-xs bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20 shrink-0 font-bold">
              #{row.rank ?? i + 1}
            </Badge>
          </div>

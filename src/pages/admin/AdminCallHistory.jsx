...
import { Phone, Clock, CheckCircle2, PhoneCall, PhoneMissed, Search, ShieldCheck, Download } from "lucide-react";
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Complete call centre call archive with recording metadata and export for audit.</p>
          </div>
          <ExportButton data={filtered} columns={exportColumns} filename="admin_call_history" label="Export" />
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={PhoneCall} label="Total Calls Logged" value={callHistoryLog.length} color="blue" />
          <StatCard icon={ShieldCheck} label="Evidence Tagged" value={callHistoryLog.filter(c => c.evidenceTagged).length} color="purple" />
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((c, i) => (
                  <tr key={i} className="hover:bg-muted/30">
                    ...
                    <td className="px-3 py-3"><Badge variant="outline" className={`text-[10px] ${c.callType === "Outbound" ? "bg-sky-50 text-sky-700" : "bg-blue-50 text-primary"}`}>{c.callType}</Badge></td>
                    ...
                    <td className="px-3 py-3"><Badge variant="outline" className={`text-xs ${c.status === "Resolved" ? "bg-emerald-50 text-emerald-700" : c.status === "Missed" ? "bg-red-50 text-red-700" : c.status === "Escalated" ? "bg-amber-50 text-amber-700" : "bg-slate-50 text-slate-600"}`}>{c.status}</Badge></td>
                    <td className="px-3 py-3">{c.evidenceTagged ? <ShieldCheck className="w-4 h-4 text-purple-600" /> : <span className="text-xs text-muted-foreground">—</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {filtered.length === 0 && <div className="text-center py-8 text-sm text-muted-foreground">No calls match your filters.</div>}
        </div>
      </div>
    </PortalLayout>
  );

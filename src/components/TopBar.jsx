...
export default function TopBar({ role = "superadmin", profile, onProfileChange, onToggleSidebar, sidebarOpen }) {
  ...
  return (
    <header className="h-14 bg-white border-b border-border flex items-center justify-between px-4 sticky top-0 z-30 shadow-sm">
      <div className="flex items-center gap-3">
        <button onClick={onToggleSidebar} className="p-2 rounded-lg hover:bg-muted text-muted-foreground transition-colors" title={sidebarOpen ? "Collapse sidebar" : "Expand sidebar"}>
          <Menu className="w-5 h-5" />
        </button>
        <div>
          <div className="text-[11px] text-muted-foreground font-medium">{info.label}</div>
          <div className="text-sm font-bold text-foreground -mt-0.5 hidden sm:block">{PORTAL_META.name}</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        {!isCitizen && (
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-muted rounded-lg text-muted-foreground">
            ...
            <input type="text" placeholder="Search complaints, officers..." className="bg-transparent text-sm w-40 lg:w-56 outline-none placeholder:text-muted-foreground/60" />
          </div>
        )}
        ...
        {profiles && (
          <div className="relative" ref={switcherRef}>
            <button onClick={() => setShowSwitcher(!showSwitcher)} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-primary bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
            ...
            {showSwitcher && (
              <div className="absolute right-0 top-11 w-64 bg-white border border-border rounded-xl shadow-xl overflow-hidden z-50">
                <div className="px-4 py-2.5 border-b border-border bg-muted/50">
                  <span className="text-xs font-semibold text-muted-foreground">Switch Profile View</span>
                </div>
                {profiles.map(p => (
                  <button key={p.id} onClick={() => handleSwitchProfile(p.id)} className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted transition-colors ${p.id === currentProfileId ? "bg-blue-50" : ""}`}>
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-[10px] font-bold">{p.avatar}</div>
                    <div className="text-left flex-1">
                      <div className="font-medium text-sm">{p.label}</div>
                      <div className="text-[11px] text-muted-foreground">{p.user}</div>
                    </div>
                    {p.id === currentProfileId && <CheckCircle2 className="w-4 h-4 text-primary" />}
                  </button>
                ))}
              </div>
            )}
        ...
        {!isSuperAdmin && (
          <div className="relative" ref={notifRef}>
            <button onClick={() => setShowNotifs(!showNotifs)} className="p-2 rounded-lg hover:bg-muted text-muted-foreground relative">
            ...
            {showNotifs && (
              <div className="absolute right-0 top-12 w-80 bg-white border border-border rounded-xl shadow-xl overflow-hidden z-50">
                <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                  ...
                  <button onClick={() => setShowNotifs(false)} className="p-1 hover:bg-muted rounded"><X className="w-3.5 h-3.5" /></button>
                </div>
                <div className="max-h-80 overflow-y-auto scrollbar-thin">
                  {notifications.map(n => (
                    <div key={n.id} className="px-4 py-3 border-b border-border last:border-0 hover:bg-muted/50 flex items-start gap-3">
        ...
        <Link to="/" className="px-3 py-1.5 text-xs font-medium text-primary bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
          Switch Portal
        </Link>

        <div className="relative" ref={profileRef}>
          <button onClick={() => setShowProfile(!showProfile)} className="flex items-center gap-2 pl-3 border-l border-border">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-xs font-bold">{info.avatar}</div>
            <div className="hidden sm:block text-left">
              <div className="text-sm font-semibold text-foreground leading-tight">{info.user}</div>
              <div className="text-[11px] text-muted-foreground leading-tight">{info.title}</div>
            </div>
          ...
          {showProfile && (
            <div className="absolute right-0 top-12 w-56 bg-white border border-border rounded-xl shadow-xl overflow-hidden z-50">
              ...
              {info.settingsPath && (
                <Link to={info.settingsPath} onClick={() => setShowProfile(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted">
                  <Settings className="w-4 h-4 text-muted-foreground" /> Settings
                </Link>
              )}
              {isSuperAdmin && (
                <Link to="/admin/users" onClick={() => setShowProfile(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-muted">
                  <Shield className="w-4 h-4 text-muted-foreground" /> Manage Access
                </Link>
              )}
              <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-destructive hover:bg-destructive/5 border-t border-border">
